function truj_create_nc_vars(inpath, infilename, outpath, outfilename, var_in, var_out)
...
unix(['ncks -O -v ' var_in ' ' inpath infilename ' ' outpath tmp_file]);
old_data = truj_read_nc([inpath infilename],  {var_in});
old_data{1}(:,:) = 0.0;
...
if isfile([outpath outfilename])
    unix(['cp ' outpath outfilename ' ' outpath tracer_file]);
end
...
unix(['cp ' outpath tracer_file ' ' outpath outfilename]);
